...
def checkpoint(epoch,loss,psnr,ssim,mse):
    ...
    #model_path1 = str(checkpoint_dir/'{}-{:.4f}-{:.4f}-{:.4f}.pth'.format(epoch,loss,psnr,ssim))
    ...
    print("Test Avg. PSNR: {:.4f} Avg. SSIM: {:.4f} MSE:{:.6f} ".format(psnr,ssim,mse))
    print("Checkpoint saved to {}".format(model_path))

    output = open(str(checkpoint_dir/'train_result.txt'),'a+')
    output.write(('{} {:.6f} {:.4f} {:.4f} {:.6f}'.format(epoch,loss,psnr,ssim,mse))+'\r\n')
    output.close()
...
def main():
    ...
    for epoch in range(args.epochs):
        loss=train(epoch+100)
        psnr,ssim,mse = test()
        checkpoint(epoch+100,loss,psnr,ssim,mse)



#---------------------------------------------------------------------------------------------------
# Training settings
parser = argparse.ArgumentParser(description='resnet&vgg16')
...
#2.set path and file
save_dir = Path('.')
checkpoint_dir = Path('.') / 'Checkpoints_v2'#save model parameters and train record
if checkpoint_dir.exists():
    print 'folder esxited'
else:
    checkpoint_dir.mkdir()

model_weights_file=checkpoint_dir/'99-0.001116-30.4385-0.8931param.pth'


#3.set dataset and dataloader
dataset=MyDataset(data_file=save_dir/'TrainData40.h5')
test_dataset=MyDataset(data_file=save_dir/'TestData40.h5')
...
model=mymodel_v2.L8()
vgg=mymodel_v2.Vgg16(requires_grad=False)
