...
def main():
    ...
    for epoch in range(args.epochs):
        loss=train(epoch+660)
        psnr,ssim,mse = test()
        checkpoint(epoch+660,loss,psnr,ssim,mse)
...
#2.set path and file
save_dir = Path('.')
checkpoint_dir = Path('.') / 'Checkpoints_high_new'#save model parameters and train record
if checkpoint_dir.exists():
    print 'folder esxited'
else:
    checkpoint_dir.mkdir()

model_weights_file=checkpoint_dir/'659-0.0013-29.9249-0.8735param.pth'
