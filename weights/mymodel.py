...
class res15(nn.Module):#h_relu4_3
    def __init__(self):
    ...
    def forward(self,inputs,vgg_feature):#
        ...
        for layer in self.resnet5:
            x1 = layer(x1)
        #print('vgg shape:',vgg_feature.size())
        ...
        diff_d=d1-d2
        if(diff_c>0):
            x2=torch.cat([x2, Variable(torch.zeros(a2,b2,diff_c,d2).cuda())], 2)
        #print('x2 shape middle:',x2.size())
        if(diff_d>0):
            x2=torch.cat([x2, Variable(torch.zeros(a2,b2,c2+diff_c,diff_d).cuda())], 3)   

        #print('x1 shape after:',x1.size())
        #print('x2 shape after:',x2.size())
